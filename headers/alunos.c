...
ALUNO * pede_aluno() {
    int erro = 0; // quando erro = 1 a funcao return NULL
    ...
    if (erro) { // erro = 1
        printf("Algum dos parametros nao e valido.\n");
        return NULL;
    }

    // Confirma dados
    if (confirmar() == 0) return NULL; // Utilizador nao confirmou
...
int inserir_aluno(lista *l, ALUNO *aluno, int toggler) {
    if (procurar_aluno(l, aluno->numero) != NULL) {
        printf("Ja existe um aluno com o numero inserido\n");
        return 0;
    }
    no_lista *novo = malloc(sizeof(no_lista)); // reserva espaco para o novo aluno
    if (novo == NULL) return 0;                // se nao conseguir reservar acaba a funcao
    novo->aluno = *aluno;                      // novo aluno e o aluno passado por parametro
    novo->prox = l->inicio;                    // o aluno asseguir ao novo agora e o inicio da lista
    l->inicio = novo;                          // o inicio e o novo aluno
